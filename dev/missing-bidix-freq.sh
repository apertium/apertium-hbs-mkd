...
cat $INPUT | grep '@' | grep '<adj>'  | sed 's/<\(f\|mi\|ma\|nt\|du\|m\|sg\|pl\|acc\|nom\|ins\|loc\|gen\|dat\|voc\)>//g' | sed 's/<\(ind\|def\)>//g' | sh ~/scripts/lowercase.sh  | sed 's/[ανρχί]/\n/g' | sed 's/^\W*\^/^/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $' | grep '<adj>' | grep '@' > $DEV/pending_adjectives.txt

cat $INPUT | grep '@' | grep '<n>'  | sed 's/\(<pl>\|<du>\|<sg>\|<ins>\|<acc>\|<dat>\|<loc>\|<gen>\|<dir>\|<nom>\|<abl>\|<abs>\|<1s>\|<2s>\|<3s>\|<1p>\|<2p>\|<3p>\|<p1s>\|<p2s>\|<p3s>\|<p1p>\|<p2p>\|<p3p>\)//g' | sed 's/<pl>//g' | sh ~/scripts/lowercase.sh  | sed 's/^\W*\^/^/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $'  > $DEV/pending_nouns.txt
