...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q https://wiki.apertium.org/wiki/Serbo-Croatian_and_Macedonian/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch https://wiki.apertium.org/wiki/Serbo-Croatian_and_Macedonian/$TESTTYPE"; fi
fi
echo "..."
